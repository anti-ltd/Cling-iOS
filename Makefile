# Cling — build/run wrapper around xcodebuild. Mirrors Clink so the muscle
# memory carries across our apps.
#
#   make run     — build + launch in the simulator
#   make device  — build + install + launch on a paired iPhone
#
# Cling is a Live Activity app: the `Cling` scheme builds the container app AND
# embeds the ClingWidgets + ClingShare extensions. Live Activity / Dynamic
# Island behavior only really shows on a physical iPhone (`make device`).
#
APP_NAME       := Cling
SCHEME         := Cling
BUNDLE_ID      := ltd.anti.cling

PROJECT        := Cling.xcodeproj
BUILD_DIR      := build
...
# App Store Connect distribution. Same API key the other apps use; the .p8
...
help:
	...
	@echo "  make device         — build + install + launch Cling on the paired iPhone"
...
# ============================================================
# App Store Connect distribution. Automatic signing:
# -allowProvisioningUpdates + the ASC API key let xcodebuild mint the Apple
# Distribution cert and App Store profiles headlessly.
#
#   make archive      — Release archive of the app + both extensions
